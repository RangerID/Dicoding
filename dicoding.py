...
customer_counts = customers_df.groupby(by="customer_state").customer_id.nunique().reset_index()
customer_counts.columns = ['customer_state', 'customer_count']
...
sns.barplot(
    x="customer_state",
    y="customer_count",
    data=customer_counts.sort_values(by="customer_count", ascending=False),
    ax=ax
)
